...
1. To be used for evaluating a model fine-tuned and saved using finetune_llama.py
2. Uses modified alpaca style prompt
'''

huggingface_hub.login(token='<token>')  # provide hugging face tokens that can access Llama models from huggingface
nltk.download('punkt')
...
if __name__ == '__main__':
	...
	dataset = load_dataset("json", data_files={"test": "<path to test data>"})   ###give the path to test data jsonl

	# dataset["test"] = dataset["test"].select(np.arange(10))
	...
	df.to_csv(args.output_dir + f'LLAMA-{model_param}-webmd-test-inference-post-finetune-' + checkpoint_name + f'-{args.max_new_tokens}.csv')
